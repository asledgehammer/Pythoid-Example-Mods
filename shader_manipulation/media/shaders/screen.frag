...
vec3 screenWorld(in vec3 pixel, in vec3 noise) {
	...
	Diffuse = blendOverlay(Diffuse,Light,LightIntensity2*Exterior);
...
void main()
{
    ...
    if(NightVisionGoggles<0.5) {
        ...
        Diffuse = desaturate(pixel, DesaturationVal);
        Diffuse = blendOverlay(Diffuse,Light,LightIntensity2);
        Diffuse = clamp(Diffuse, 0.0, 1.0);
        intensity = (0.299 * Diffuse.r) + (0.587 * Diffuse.g) + (0.114 * Diffuse.b);

        //boost based on darkness (= dark color with high intensity)
        float str = (((1.0-Light.r)+(1.0-Light.g)+(1.0-Light.b))*0.333)*LightIntensity2;
        if(Exterior>0.5) {
...
/*void main() LAST MAIN I
{
    ...
    //Diffuse += pixel;
        ...
        //Diffuse += Diffuse * Light * LightIntensity2 * clamp(pixel.b,0.25,1.0); //LightIntensity2;
    //Diffuse += Light * LightIntensity2 * Diffuse * clamp(pixel.b,0.25,1.0);
    //previous screenshot
        //Diffuse = desaturate(pixel, DesaturationVal);
        //Diffuse += Light * LightIntensity2 * clamp((pixel.r+pixel.g+pixel.b)*0.34,0.0,1.0);
    float intensity;
    if(NightVisionGoggles<0.5) {
        Diffuse = desaturate(pixel, DesaturationVal);
        Diffuse = blendOverlay(Diffuse,Light,LightIntensity2);
        //Diffuse = blendOverlay(pixel,Light,LightIntensity2);
